...
- `source` — type (`pdf`|`text`|`lesson_audio`|`voice_question`|`gutenberg`|`mochi`|`manual`|`chat`|`suggestion`),
  ...
  Lessons list for the "· N min" row label), language (TEXT, `es`|`en`; the
  source's language, set at creation — pdf/lesson_audio/voice_question `es`,
  text by `detectLanguage()`, gutenberg `en`; existing rows backfilled `es`
  by migration 005). The triage→word-materialization path
  (`triage-queries.ts`) reads it to route kept words into the
  matching-language deck and to dedupe against same-language words,
  replacing the former Spanish-only hardcode.
...
- 2026-06-13 — Schema gate (orchestrator-approved, iteration 152; nothing else in flight): `source.language` (TEXT `es`|`en`) added via migration 005 — additive `ALTER TABLE source ADD COLUMN` with existing rows backfilled to `es` (no table rebuild, like 004). Unblocks Phase 3 (Gutenberg/KJV → English deck): the triage/confirm/dedupe/deck-routing path in `triage-queries.ts` was hardcoded `LANGUAGE="es"` (every kept word → the Spanish deck); it now reads `source.language` so English (gutenberg/KJV) words land in the English Vocabulary deck and dedupe against English words. Language is set at all source-creation sites. Requested by: Phase-3 decomposition (orchestrator). Outcome: approved — additive, within §8, no §13 trigger. Task: `schema-gate-005` (ORCH_MODEL/high, runs alone).
