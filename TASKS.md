...
- **mobile-nav-and-review-landing**: (a) build **AppNav** — the phone bottom-bar nav specified in `design/screens/shell.md` (fixed bottom, `--hit-target` tall, `--color-paper` + `--color-rule` top hairline, four items Home·Review·Library·Grammar, active = `--color-accent` + `--color-accent-wash` underline-pill); never built (home-nav-footer shipped SiteHeader/SiteFooter only), so phone has ZERO nav today. Mount in `AppShell` on phone widths only. (b) Route `/review` through `AppShell` (like Quiz) so its resting states (pre-session landing "N due → Start review", empty, finished) show standard chrome while the active card-answering run stays the focused session takeover. NEW web/src/components/AppNav.{tsx,css,test.tsx} + index.ts export; EDIT AppShell.tsx, App.tsx, Review.tsx, Review.css. Includes: design/INDEX.md, design/tokens.md, design/screens/shell.md, design/screens/review.md. Schema changes allowed: none. (spawned: 2026-06-12, standard) [Must — FEEDBACK]
- **library-forgot-button-layout**: the "I forgot this" control overlaps the "LEARNING" status text in the `/library` word row (`.library__row-status` vs `.library__row-forgot`); lay them out horizontally (button beside the status, no overlap), existing tokens only. Touches web/src/screens/Library.css (+ Library.tsx markup / WordDetail.* if it also overlaps there). Includes: design/INDEX.md, design/tokens.md, design/screens/library.md. Schema changes allowed: none. (spawned: 2026-06-12, `--model sonnet --effort medium`) [Should — trivial CSS; file-disjoint from mobile-nav]
- **transcription-layer**: `TranscriptionProvider` adapter (server-side only, `/server/src/transcription/`) mirroring `LlmProvider`; OpenAI Whisper default adapter (api.openai.com, OPENAI_API_KEY); ~60-min chunking + stitch; per-lesson upfront cost estimate (~$0.40/hr); `transcription_call` logging + spend on the System page; mocked-provider tests. Does NOT build audio upload/jobs (that is lesson-recording-ingestion). transcription_call already in 001 to spec → Schema changes allowed: none. Includes: design/INDEX.md, design/tokens.md, design/screens/system.md. (spawned: 2026-06-12, `--model "$ORCH_MODEL" --effort high`) [Must — Phase 2, build first; riskiest; file-disjoint — server/ + System screen only]
...
- **notes-on-answers** — NEW FEATURE (FEEDBACK): per-answer self-note on any lesson/quiz question (writes the `note` table — created by schema-gate-003, now landed), browsable, fed back as context into future quiz/lesson generation. Gate integrated iter 90 → UNBLOCKED; awaiting capacity. No schema grant needed (Schema changes allowed: none). NOTE next-spawn: file-independence vs ask-chatbot/suggestions — notes touches the Quiz/Lesson answer screens + a notes browse surface; check overlap before spawning together [Should]
...
<!-- transcription-layer → In Progress (spawned iteration 90, gate lifted) -->
...
- **ask-chatbot** — persistent ChatThread/ChatMessage with page-context seeding, plain "Ask" button (no floating bubble), server-side tool set (add_word_to_deck, lookup_word, get_page_context) with inline mutation confirmation, threads survive restarts, all turns through llm/service.ts + llm_call logging. Design ready (screens/ask.md + ChatTurn/ToolConfirm/RecordButton). Chat tables landed (schema-gate-003, iter 90) → UNBLOCKED; awaiting capacity. No schema grant needed [Must — Phase 2]
- **suggestions** — Suggestion table (uniqueness enforced — nothing re-suggested, skips included), LLM-selected one-at-a-time word/grammar-topic proposals calibrated on known/mastered words + mastery, add/skip, pool-exhausted empty state. Design ready (screens/suggestions.md). Suggestion table landed (schema-gate-003, iter 90) → UNBLOCKED; awaiting capacity. No schema grant needed [Must — Phase 2]
