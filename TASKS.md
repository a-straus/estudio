...
- **quiz-caching-and-bank**: reuse an existing unflagged cached `quiz_question` before regenerating (review-03 S1 / GOAL §6.4/§6.7); widen the def_match distractor pool + §6.4 distractor-quality rule (similar level, never a synonym — nit#10); pass `quiz_question_id` for ALL quiz-origin misses incl. def_match (S6). SERVER-ONLY: owns `server/src/jobs/quizGen.ts` + `server/src/routes/quiz.ts`. Schema changes allowed: none. (re-spawned: 2026-06-11 iter 61 — was STALE w/ 0 commits, plain spawn)
- **ingestion-progress-and-topic-link**: real progress on PDF page reading (per-page N of M) and curriculum building, reported by the jobs and rendered on `/ingest` + `/grammar`; plus review-03 S12 deterministic page→topic linking from the page-classification LLM (not the filename heuristic), NULL when unconfident. Owns `server/src/jobs/pdfIngestion.ts` + `server/src/jobs/grammarSeed.ts` + `prompts/page_classification.md` + the progress bits of `web/src/screens/Ingest.{tsx,css}` + `Grammar.{tsx,css}` (+ `ingestApi.ts`/`grammarApi.ts` if needed). Schema changes allowed: none (uses existing `job.progress` TEXT). (spawned: 2026-06-11 iter 61, plain spawn, `--include design/INDEX.md --include design/tokens.md`)
