...
- **schema-gate-003**: SCHEMA GATE migration (runs ALONE — orchestrator-approved batch, iteration 88): migration 003 = new `note` table + `quiz_attempt.style` CHECK gains 'mixed' (and code stops writing fabricated styles for mixed/lesson attempts) + materialize the already-specified Phase-2 tables `transcription_call`, `chat_thread`, `chat_message`, `suggestion`. Unblocks notes-on-answers, quiz-attempt-style-mixed (folded in), transcription-layer, ask-chatbot, suggestions — all WITHOUT schema grants. (spawned: 2026-06-12, `--model "$ORCH_MODEL" --effort high`, no --include)
...
- **flaky-test watch** — one unreproduced single-test failure on the first post-merge check.sh run (iteration 88); two subsequent runs + a direct vitest run all green (408 tests). If it recurs, identify and fix the flaky test [watch item, no spawn yet]
...
- **transcription-layer** — `TranscriptionProvider` adapter interface mirroring LlmProvider (server-side only, key in .env), Whisper-class default adapter, chunking for ~60-min audio, stitch, per-lesson upfront cost estimate (~$0.40/hr), `transcription_call` logging + spend on System page; mocked-provider tests. Prereqs: (a) schema-gate-003 creates `transcription_call` — WAIT for it, then no schema grant needed; (b) firewall host + OPENAI_API_KEY → [PENDING] question posted in QUESTIONS.md iteration 88 (blocks LIVE use only; construction + mocked tests don't wait) [Must — Phase 2, build first; riskiest]
