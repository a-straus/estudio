...
- **ingestion-progress-and-topic-link**: real progress on PDF page reading (per-page N of M) and curriculum building, reported by the jobs and rendered on `/ingest` + `/grammar`; plus review-03 S12 deterministic page→topic linking from the page-classification LLM (not the filename heuristic), NULL when unconfident. Owns `server/src/jobs/pdfIngestion.ts` + `server/src/jobs/grammarSeed.ts` + `prompts/page_classification.md` + the progress bits of `web/src/screens/Ingest.{tsx,css}` + `Grammar.{tsx,css}` (+ `ingestApi.ts`/`grammarApi.ts` if needed). Schema changes allowed: none (uses existing `job.progress` TEXT). (re-spawned FRESH: 2026-06-11 iter 67 — abandoned prior branch after 2 STALE runs both lost rabbit-holing on `better_sqlite3`/node-gyp rebuilds w/ 0 commits; new brief forbids native rebuilds, `--effort high --include design/INDEX.md --include design/tokens.md`)
- **system-preferences**: add System screen Preferences section (review-03 S8) — "Definitions on reveal · Spanish/English/Both" + "New cards per day · 10/20/40" SegmentedControls backed by GET/PUT `/api/settings` (setting table; no schema change); definitionDisplay wired into Review (and ideally Quiz) reveal; nit#5 spend time-window + errors pager. Route stub pre-partitioned on base (commit e15ab3c) so worker owns settings.ts + System screen, not app.ts. (spawned: 2026-06-11 iter 66, `--include design/INDEX.md --include design/tokens.md --include design/screens/system.md --include design/components.md`)
...
- **design-polish-foundation** — READY TO SPAWN once both in-flight Phase-1 fixes (ingestion-progress, system-preferences) land and capacity is free; runs ALONE (touches the token stylesheet + every base component + shell, so it conflicts with any screen/component task). `spawn --model "$ORCH_MODEL" --effort high --include design/INDEX.md --include design/tokens.md --include design/components.md --include design/interaction.md --include design/screens/shell.md --include design/screens/home.md`: materialize the new additive tokens (display scale, --space-9/-10, --shadow-3, --color-paper-sunken, --color-accent-strong, --header-height, --motion-slow + dark overrides) into the token stylesheet; restyle the base component library to the elevated bar; build the new chrome components (SiteHeader, SiteFooter) + home components (HomeHero, OverviewCard); so the existing screens inherit the new look. (Home screen wiring + AppNav reduction + footer mounting = the home-nav-footer task next, since it touches App.tsx routing.) [Must]
