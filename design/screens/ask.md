...
- _Deleting a thread:_ the row's trailing `×` deletes that conversation and its messages immediately, with no confirmation dialog (owner-chosen — threads are disposable scratch space; any word they spawned is already saved in the deck). The row leaves the list in place; if the currently-open thread is the one deleted, return to the list.
